...
# centaurid config node "tcp://127.0.0.1:99885"
...
sed -i'' -e 's/26657/50001/g' ~/.banksy/config/config.toml ~/.banksy/config/client.toml
sed -i'' -e 's/26656/50000/g' ~/.banksy/config/config.toml ~/.banksy/config/client.toml
...
centaurid start --pruning=nothing  --minimum-gas-prices=0stake --log_level=debug
